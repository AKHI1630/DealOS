...
import { LayoutDashboard, Users, Settings as SettingsIcon, Inbox, Bot, Zap, Plus, Lock, Moon, Sun, User } from 'lucide-react';
...
export default function Sidebar({ 
  ...
  const { isConfigComplete, campaigns, currentCampaignId, setCurrentCampaignId, theme, setTheme, settings } = useAppContext();
  ...
  const confirmNewCampaign = () => {
    ...
    navigate('/config', { state: { reset: true } });
  };
    ...
    <>
      <div className="w-64 border-r border-app-border bg-app-card flex flex-col h-full shrink-0">
        ...
        <div className="px-4 mb-2">
          <div className="text-xs font-semibold text-app-muted uppercase tracking-wider mb-2 mt-2">Campaigns</div>
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {campaigns.map(camp => (
              <button
                key={camp.id}
                onClick={() => setCurrentCampaignId(camp.id)}
                className={cn(
                  "w-full flex items-center justify-between px-2 py-1.5 rounded-md text-sm transition-colors text-left",
                  currentCampaignId === camp.id ? "bg-app-bg text-app-text font-medium" : "text-app-muted hover:bg-app-bg hover:text-app-text"
                )}
              >
                <span className="truncate pr-2">{camp.name}</span>
                <span className="flex items-center gap-1 shrink-0 text-[10px]">
                  {camp.status === 'Active' && <span className="w-2 h-2 rounded-full bg-emerald-500" title="Active" />}
                  {camp.status === 'Complete' && <span className="w-2 h-2 rounded-full bg-blue-500" title="Complete" />}
                  {camp.status === 'Paused' && <span className="w-2 h-2 rounded-full bg-amber-500" title="Paused" />}
                  {camp.status === 'Draft' && <span className="w-2 h-2 rounded-full bg-zinc-400" title="Draft" />}
                </span>
              </button>
            ))}
          </div>
        ...
        <div className="p-4 border-t border-app-border space-y-2">
          ...
          >
            ...
            <div className={cn(
              ...
              isDemoMode ? "bg-app-accent" : "bg-zinc-300 dark:bg-zinc-700"
            )}>
              <motion.div 
          ...
          <div className="flex items-center justify-between px-3 py-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-app-text truncate max-w-[100px]">{settings.profile.name}</span>
            </div>
            <button 
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-app-muted text-sm mb-6">
                Complete your business configuration first to unlock this feature.
              </p>
        ...
        {showNewCampaignModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-app-muted text-sm mb-6">
                This will save your current campaign and start fresh.
              </p>
              <div className="flex justify-end gap-3">
                <button 
